...
from shared.services.service_order_core import safe_float, get_base_unit_cost, read_order_table
...
def validate_order_submission(
    *,
    ...
    prices_df_for_check = read_order_table("prices")
    for row in submit_rows:
        if safe_float(row["order_qty"]) <= 0:
            continue
