...
def get_existing_stock_line_id_map(stocktake_id: str) -> dict[str, str]:
    """Returns {item_id: stocktake_line_id} for an existing stocktake."""
    if not stocktake_id:
        return {}
    ...
    for _, row in work.iterrows():
        item_id = norm(row.get("item_id", ""))
        line_id = norm(row.get("stocktake_line_id", ""))
