...
def get_existing_order_maps(po_id: str) -> tuple[dict[str, float], dict[str, str]]:
    ...
    pol_df = read_order_table("purchase_order_lines")
    ...
    for _, row in work.iterrows():
        ...
        if not item_id:
            continue
        qty_map[item_id] = safe_float(row.get("order_qty", row.get("qty", 0)))
        unit_map[item_id] = norm(row.get("order_unit", row.get("unit_id", "")))
    return qty_map, unit_map


def get_existing_po_line_id_map(po_id: str) -> dict[str, str]:
    """Returns {item_id: po_line_id} for an existing purchase order."""
    if not po_id:
        return {}
    ...
    for _, row in work.iterrows():
        item_id = norm(row.get("item_id", ""))
        line_id = norm(row.get("po_line_id", ""))
        if item_id and line_id:
