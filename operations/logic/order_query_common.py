...
from shared.services.service_order_core import get_order_table_versions, read_order_table
...
def load_selector_tables() -> dict[str, pd.DataFrame]:
    versions = get_order_table_versions(SELECTOR_TABLES)
    ...
    data = {name: read_order_table(name) for name in SELECTOR_TABLES}
...
def load_order_page_tables() -> dict[str, pd.DataFrame]:
    versions = get_order_table_versions(ORDER_PAGE_TABLES)
    ...
    data = {name: read_order_table(name) for name in ORDER_PAGE_TABLES}
