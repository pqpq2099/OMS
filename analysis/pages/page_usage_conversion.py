...
def page_usage_conversion():
    ...
    uploaded = st.file_uploader(
        ...
        type=["xlsx"],
        key="usage_upload",
        help="支援 report_UberEats_xxx.xlsx 或 report_foodpanda_xxx.xlsx",
    )

    if not uploaded:
        st.info("請上傳 report_UberEats_YYYYMMDD_YYYYMMDD.xlsx 或 report_foodpanda_YYYYMMDD_YYYYMMDD.xlsx")
        return
    ...
    with col_left:
        st.markdown(f"**平台：** {result['platform']}")
        st.markdown(f"**區間：** {result['date_range']}")
        st.markdown("---")

        sales = result["sales_df"]
        st.markdown(f"**銷售品項（{len(sales)} 項）**")
        st.dataframe(
