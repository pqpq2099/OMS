-- =============================================================================
-- 003_order_write_rpc.sql  —  原子寫入 RPC function
-- 建立時間: 2026-03-30
-- 說明:
--   1. 為 stocktake_lines / purchase_order_lines 補 partial unique index（防禦用）
--   2. 建立 rpc_save_order_transaction(p_payload jsonb)
--      — 在單次 DB transaction 內完成 4 張表 upsert + audit_logs insert
--   3. 收斂執行權限：只有 service_role 可呼叫
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Part 1：Partial unique index（補強；PK 已存在，此為額外防禦層）
-- -----------------------------------------------------------------------------
