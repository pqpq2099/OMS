-- =============================================================================
-- 009_rpc_add_po_stocktake_id.sql  —  RPC function 補寫 purchase_orders.stocktake_id
-- 建立時間: 2026-03-31
-- 說明:
--   008 已新增 purchase_orders.stocktake_id 欄位。
--   本 migration 更新 rpc_save_order_transaction，
--   在 purchase_orders INSERT/UPDATE 時同步寫入 stocktake_id。
--   其餘邏輯（null guard / partial index / 權限）均不變。
-- =============================================================================
...
BEGIN
    ...
    -- 3. Upsert purchase_order（含 stocktake_id 正式關聯）
