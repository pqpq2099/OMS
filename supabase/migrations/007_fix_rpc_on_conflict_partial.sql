-- =============================================================================
-- 007_fix_rpc_on_conflict_partial.sql
-- 建立時間: 2026-03-30
-- 說明: 更新 rpc_save_order_transaction，
--       將 stocktake_lines / purchase_order_lines 的 ON CONFLICT 改為
--       partial index 語法（WHERE 條件），與 006 建立的 partial unique index 對齊。
-- =============================================================================
...
BEGIN
    ...
    -- 2. Upsert stocktake_lines（ON CONFLICT 對齊 partial index）
    ...
    -- 4. Upsert purchase_order_lines（ON CONFLICT 對齊 partial index）
