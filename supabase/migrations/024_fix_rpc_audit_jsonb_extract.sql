-- =============================================================================
-- 024_fix_rpc_audit_jsonb_extract.sql
-- 建立時間: 2026-04-04
-- 說明:
--   migration 015 重寫 rpc_save_order_transaction 時，audit_logs 段落的
--   before_json / after_json 誤用 ->> (text 萃取) 而非 -> (jsonb 萃取)，
--   導致 PostgreSQL 報錯：
--     column "before_json" is of type jsonb but expression is of type text (42804)
--
--   本 migration 修正：
--     v_row->>'before_json'  →  v_row->'before_json'
--     v_row->>'after_json'   →  v_row->'after_json'
--
--   其餘邏輯與 015 完全相同，無其他變動。
-- =============================================================================
...
BEGIN
    ...
    IF v_st IS NOT NULL AND v_st != 'null'::jsonb THEN
        ...
        ON CONFLICT (stocktake_id) DO UPDATE SET
            ...
            -- stocktake_type 不加入 DO UPDATE：保留初次建立時的 type
    END IF;
    ...
    -- 5. Insert audit_logs
    --    [FIX] -> (jsonb 萃取) 而非 ->> (text 萃取)，避免 42804 type mismatch
