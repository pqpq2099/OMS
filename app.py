...
if st.session_state.step == "select_store":
    ...
    st.write("<b>📦 作業廠商 (雙欄對齊)</b>", unsafe_allow_html=True)
    
    if df_i is not None:
        vendors = sorted(df_i['廠商名稱'].unique())
        # 💡 物理性強制雙欄：透過每兩個 vendor 分一組
    ...
    # 標題列：物理性極致對齊比例 5.5 : 1.75 : 1.75
    h1, h2, h3 = st.columns([5.5, 1.75, 1.75])
    ...
    with st.form("inventory_form"):
        ...
        for _, row in items.iterrows():
            ...
            c1, c2, c3 = st.columns([5.5, 1.75, 1.75])
            with c1:
...
# 其他分頁邏輯保持一致... (analysis, export, view_history 同前版)
