...
# ============================================================
# [A2] Global UI Style - 【重點修正：強行鎖定三欄同列】
# ============================================================
def apply_global_style():
    ...
    st.markdown("""
        ...
        /* 2. 【核心鎖定】強制 fill_items 頁面的三欄同列邏輯 */
        ...
        /* 4. 字體微調：確保手機閱讀不跑版 */
        .stCaption { font-size: 10px !important; line-height: 1.2 !important; }
        b { font-size: 14px !important; }
        </style>
        """, unsafe_allow_html=True)

# ============================================================
# [B1/C1] 資料引擎 (維持原樣，確保事實鎖定)
# ============================================================
@st.cache_resource
def get_gspread_client():
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    try:
        creds = ServiceAccountCredentials.from_json_keyfile_dict(st.secrets["gcp_service_account"], scope) if "gcp_service_account" in st.secrets else ServiceAccountCredentials.from_json_keyfile_name("service_account.json", scope)
...
def page_fill_items(df_i, df_pr, item_map):
    ...
    h1.write("<b>品項</b>", unsafe_allow_html=True)
    ...
    with st.form("inventory_form", clear_on_submit=False):
        ...
        for _, row in items.iterrows():
            ...
            with c1:
                ...
                st.caption(f"前:{p_s:.1f} | 單:${price:.1f} | 💡:{suggest_qty:.1f}")
            with c2:
        ...
        if st.form_submit_button("💾 儲存Facts事實", use_container_width=True):
            ...
            if valid_events:
                ...
                st.success(f"✅ 已紀錄 {len(valid_events)} 項異動事實"); st.session_state.step = "select_vendor"; st.rerun()
...
def main():
    ...
    if st.session_state.step == "select_store": page_select_store(df_s)
    elif st.session_state.step == "select_vendor": page_select_vendor(df_i)
    elif st.session_state.step == "fill_items": page_fill_items(df_i, df_pr, None)
    elif st.session_state.step == "analysis": # 略
        pass
