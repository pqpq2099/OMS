...
def page_fill_items(df_i: pd.DataFrame | None, df_pr: pd.DataFrame | None, item_display_map: dict):
    st.markdown(
        "<style>.block-container { padding-top: 2rem !important; padding-left: 0.3rem !important; padding-right: 0.3rem !important; } "
        ...
        "div[data-testid='stHorizontalBlock'] > div:nth-child(2), div[data-testid='stHorizontalBlock'] > div:nth-child(3) { flex: 0 0 72px !important; min-width: 72px !important; max-width: 72px !important; } "
        "div[data-testid='stNumberInput'] label { display: none !important; }</style>",
        unsafe_allow_html=True,
    )
        ...
        for f_id in items["品項ID"].unique():
            ...
            if not past.empty:
                ...
                if p_p_val > 0 or p_u_val > 0:
                    ref_list.append(
                        {"品項名稱": f_name, "上次叫貨": round(float(p_p_val), 1), "期間消耗": round(float(p_u_val), 1)}
                    )
    ...
    with st.form("inventory_form"):
        ...
        for _, row in items.iterrows():
            ...
            with c2:
                t_s = st.number_input(
                    "庫", min_value=0.0, step=0.1, key=f"s_{f_id}", format="%g", value=None, label_visibility="collapsed"
                )
            with c3:
                t_p = st.number_input(
                    "進", min_value=0.0, step=0.1, key=f"p_{f_id}", format="%g", value=None, label_visibility="collapsed"
                )
...
# ============================================================
# [E4] view_history - 歷史紀錄查詢（✅圖表改成報表模式：只有圖片+下載）
# ============================================================
def page_view_history():
    st.markdown(
        "<style>[data-testid='stMainBlockContainer'] { max-width: 95% !important; padding-left: 0.5rem !important; padding-right: 0.5rem !important; } "
        "[data-testid='stDataFrame'] [role='gridcell'] { padding: 1px 2px !important; line-height: 1.0 !important; } "
        "[data-testid='stDataFrame'] [role='columnheader'] { padding: 2px 2px !important; font-size: 10px !important; }</style>",
        unsafe_allow_html=True,
    )
    ...
    with t2:
        if not HAS_PLOTLY:
            ...
            # ✅ 報表模式 config：禁用拖拉/縮放/框選，只留圖片與下載(PNG 相機)
            PLOTLY_CONFIG = {
                "displayModeBar": True,
                "displaylogo": False,
                "scrollZoom": False,
            ...
            if not p_df.empty:
                fig = px.line(
                    ...
                    y="期間消耗",
                    markers=True,
                    title=f"📈 【{sel_i_h}】消耗趨勢",
                )
                fig.update_layout(dragmode=False)  # ✅ 禁用拖拉
...
def page_analysis():
    ...
    st.markdown(
        ...
        f"<div style='flex: 1; padding: 10px; border-radius: 8px; border-left: 4px solid #4A90E2; background: rgba(74, 144, 226, 0.05);'>"
        f"<div style='font-size: 11px; font-weight: 700; opacity: 0.8;'>💰 採購總額 ({selected_v})</div>"
        f"<div style='font-size: 18px; font-weight: 800; color: #4A90E2;'>${total_buy:,.1f}</div></div>"
        f"<div style='flex: 1; padding: 10px; border-radius: 8px; border-left: 4px solid #50C878; background: rgba(80, 200, 120, 0.05);'>"
        f"<div style='font-size: 11px; font-weight: 700; opacity: 0.8;'>📦 庫存殘值估計</div>"
        f"<div style='font-size: 18px; font-weight: 800; color: #50C878;'>${total_stock_value:,.1f}</div></div></div>",
        unsafe_allow_html=True,
    )
    ...
    with t_trend:
        if HAS_PLOTLY:
            ...
            if not trend_daily.empty:
                st.plotly_chart(
                    px.line(trend_daily, x="日期_dt", y="總金額", markers=True, title="📈 採購金額趨勢"),
                    use_container_width=True,
                )
            rank_df = (
            ...
            if not rank_df.empty:
                st.plotly_chart(
                    px.bar(rank_df, x="品項名稱", y="總金額", title="📊 品項採購金額排行 (Top 20)"),
                    use_container_width=True,
                )
