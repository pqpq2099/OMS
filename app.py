...
st.markdown("""
    ...
    /* 2. 徹底消滅 arr_right/down 幽靈文字 (物理性移除組件 Label 空間) */
...
# --- 步驟 2：廠商中心 (確保功能按鈕可見) ---
    ...
    st.write("<b>📦 作業廠商</b>", unsafe_allow_html=True)
    if df_i is not None:
    ...
    st.write("<b>🛠️ 報表中心</b>", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
...
# --- 步驟 3：填寫頁面 (強制單排、移除 (前結:)) ---
    ...
    if not hist_df.empty:
        ...
        if ref_list:
            with st.expander("📊 歷史參考數據 (前結)", expanded=True):
