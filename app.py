    ...
    if not hist_df.empty:
        ...
        if not recs.empty:
            ...
            if st.button("🚀 直接發送明細至 LINE", type="primary", use_container_width=True):
                if send_line_message(output): st.success(f"✅ 已成功推送到【{st.session_state.store}】群組！")
                else: st.error("❌ 發送失敗，請檢查設定。")
        else: st.info("💡 今日尚無叫貨紀錄。")
...
def page_analysis():
    ...
    v_filt = filt.copy()
    if selected_v != "全部廠商": v_filt = v_filt[v_filt["廠商"] == selected_v]
    ...
    if selected_item != "全部品項": final_filt = final_filt[final_filt["品項名稱"] == selected_item]
    total_buy = final_filt["總金額"].sum() if "總金額" in final_filt.columns else 0
    last_stock = final_filt.sort_values("日期").groupby("品項名稱").tail(1)
    total_stock_value = (last_stock["本次剩餘"] * last_stock["單價"]).sum() if ("本次剩餘" in last_stock.columns and "單價" in last_stock.columns) else 0
    st.markdown(f"<div style='display: flex; gap: 10px; margin-bottom: 20px;'><div style='flex: 1; padding: 10px; border-radius: 8px; border-left: 4px solid #4A90E2; background: rgba(74, 144, 226, 0.05);'><div style='font-size: 11px; font-weight: 700; opacity: 0.8;'>💰 採購總額 ({selected_v})</div><div style='font-size: 18px; font-weight: 800; color: #4A90E2;'>${total_buy:,.1f}</div></div><div style='flex: 1; padding: 10px; border-radius: 8px; border-left: 4px solid #50C878; background: rgba(80, 200, 120, 0.05);'><div style='font-size: 11px; font-weight: 700; opacity: 0.8;'>📦 庫存殘值估計</div><div style='font-size: 18px; font-weight: 800; color: #50C878;'>${total_stock_value:,.1f}</div></div></div>", unsafe_allow_html=True)
    if final_filt.empty:
        st.info("💡 尚未產生分析資料"); return
    t_detail, t_trend = st.tabs(["📋 明細", "📈 趨勢"])
    with t_detail:
        st.write("<b>📋 進銷存匯總明細</b>", unsafe_allow_html=True)
        required_cols = {"廠商", "品項名稱", "單位", "單價", "期間消耗", "本次叫貨", "總金額"}
        if not required_cols.issubset(set(final_filt.columns)): st.warning("⚠️ 欄位不足")
        else:
    ...
    with t_trend:
        if HAS_PLOTLY:
            ...
            trend_daily = trend_df.dropna(subset=["日期_dt"]).groupby("日期_dt", as_index=False)["總金額"].sum().sort_values("日期_dt")
            if not trend_daily.empty:
                st.plotly_chart(px.line(trend_daily, x="日期_dt", y="總金額", markers=True, title="📈 採購金額趨勢"), use_container_width=True)
            rank_df = final_filt.groupby("品項名稱", as_index=False)["總金額"].sum().sort_values("總金額", ascending=False).head(20)
            if not rank_df.empty:
                st.plotly_chart(px.bar(rank_df, x="品項名稱", y="總金額", title="📊 品項採購金額排行 (Top 20)"), use_container_width=True)
...
def router(df_s, df_i, df_pr, item_display_map):
    ...
    elif s == "fill_items": page_fill_items(df_i, df_pr, item_display_map)
    elif s == "view_history": page_view_history()
    elif s == "export": page_export()
...
# ============================================================
def main():
    apply_global_style(); init_session()
