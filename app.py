...
st.markdown("""
    ...
    /* 標題與廠商按鈕 */
    h1, h2, h3 { font-weight: 800 !important; margin-bottom: 1rem !important; }
...
if st.session_state.step == "select_store":
    ...
    # --- 區塊 2：數據與報表中心 (戰略區) ---
    # 使用 container 包裹，確保不與上方廠商按鈕混淆
    with st.container():
        st.markdown('<div class="function-box">', unsafe_allow_html=True)
        st.write("<b>🛠️ 系統功能與報表</b>", unsafe_allow_html=True)
        ...
        if st.button(f"📜 查看 {st.session_state.store} 歷史庫", use_container_width=True):
            st.session_state.view_df = get_worksheet_data(history_sheet)
            st.session_state.step = "view_history"; st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        
    if st.button("⬅️ 返回分店", use_container_width=True):
    ...
    if st.button("⬅️ 返回列表", use_container_width=True): st.session_state.step = "select_vendor"; st.rerun()

elif st.session_state.step == "view_history":
    # 歷史紀錄數據庫 (與之前優化的 Tab 邏輯一致)
