    ...
    if df_i is not None:
        ...
        for v in vendors:
            if st.button(f"📦 {v}", key=f"v_{v}", use_container_width=True):
                ...
                st.session_state.history_df = get_cloud_data() # 💡 選擇廠商後立馬抓歷史
                st.session_state.step = "fill_items"; st.rerun()
    
    st.write("---")
    st.write("<b>📊 功能控制中心</b>", unsafe_allow_html=True)
...
# --- 步驟 3：填寫盤點品項 ---
    ...
    # 💡 關鍵修正：如果這一步沒有 vendor 資訊，強制回退防止空白
    ...
    with st.form("inventory_form"):
        ...
        if st.form_submit_button("💾 儲存並同步至雲端", use_container_width=True):
...
# (歷史紀錄、分析與報表頁面邏輯維持，確保結構不崩潰)
    ...
    if not view_df.empty:
        ...
        with tab1:
            search = st.text_input("🔍 搜尋品項")
            display_df = view_df.copy()
            if search: display_df = display_df[display_df.astype(str).apply(lambda x: x.str.contains(search)).any(axis=1)]
            st.dataframe(display_df.sort_values('日期', ascending=False), use_container_width=True, hide_index=True)
        with tab2:
            if HAS_PLOTLY:
                target = st.selectbox("分析品項", options=sorted(view_df['品項名稱'].unique()))
                plot_df = view_df[view_df['品項名稱'] == target].sort_values('日期')
