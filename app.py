...
def nav_button(label: str, step: str, key: str, primary: bool = False):
    if st.button(label, use_container_width=True, type="primary" if primary else "secondary", key=key):
        go(step)
...
def page_select_vendor(df_i: pd.DataFrame | None):
    ...
    if st.button("📄 產生今日進貨明細", type="primary", use_container_width=True, key="nav_export"):
        st.session_state.history_df = get_cloud_data()
        go("export")

    if st.button("📈 期間進銷存分析", use_container_width=True, key="nav_analysis"):
        st.session_state.history_df = get_cloud_data()
        go("analysis")

    history_sheet = f"{st.session_state.store}_紀錄"
    if st.button("📜 查看分店歷史紀錄", use_container_width=True, key="nav_view_history"):
        st.session_state.view_df = get_worksheet_data(history_sheet)
        go("view_history")

    nav_button("⬅️ 返回分店列表", "select_store", key="back_to_store")
...
def page_fill_items(df_i: pd.DataFrame | None, df_pr: pd.DataFrame | None, item_display_map: dict):
    ...
    if df_i is None or df_i.empty:
        ...
        nav_button("⬅️ 返回功能選單", "select_vendor", key="back_from_fill_err")
        return
    ...
    if not hist_df.empty:
        ...
        for f_id in items["品項ID"].unique():
            f_name = item_display_map.get(str(f_id).strip(), "")
    ...
    nav_button("⬅️ 返回功能選單", "select_vendor", key="back_from_fill")
...
def page_view_history():
    ...
    if v_df.empty:
        st.info("💡 尚無歷史紀錄可供查看。")
        nav_button("⬅️ 返回", "select_vendor", key="back_from_history_empty")
        return
    ...
    nav_button("⬅️ 返回", "select_vendor", key="back_hist_final")
...
def page_export():
    ...
    if hist_df.empty:
        st.error("❌ 無法讀取 Records。")
        nav_button("⬅️ 返回選單", "select_vendor", key="back_to_vendor_export_err")
        return
    ...
    if recs.empty:
        st.info("💡 今日尚無叫貨紀錄。")
        nav_button("⬅️ 返回選單", "select_vendor", key="back_to_vendor_export_empty")
        return
    ...
    if st.button("🚀 直接發送明細至 LINE", type="primary", use_container_width=True, key="push_line_export"):
        if send_line_message(output):
            st.success(f"✅ 已成功推送到【{st.session_state.store}】群組！")
        else:
            st.error("❌ 發送失敗，請檢查該店 ID 已填入 Secrets 且機器人在群組內。")

    nav_button("⬅️ 返回選單", "select_vendor", key="back_to_vendor_export")
...
def page_analysis():
    ...
    if a_df.empty:
        ...
        nav_button("⬅️ 返回選單", "select_vendor", key="back_from_analysis_empty")
        return
    ...
    if filt.empty:
        ...
        nav_button("⬅️ 返回選單", "select_vendor", key="back_from_analysis_no_data")
        return
    ...
    if final_filt is None or len(final_filt) == 0:
        st.info("💡 尚未產生分析資料")
        nav_button("⬅️ 返回選單", "select_vendor", key="back_from_analysis_no_final")
        return
    ...
    nav_button("⬅️ 返回選單", "select_vendor", key="back_from_analysis")
